...
function toggle_order_confirm_panel(type) {
    ...
    var langu = document.getElementById("langu").innerHTML;
...
function addorder() {
    ...
    var order_ji = $('.period-widget-view .active').attr('data-ji');

    if (order_ji == 1) {
        var ji_min_price = 1000;
        var ji_max_price = 49999;
    } else if (order_ji == 2) {
        var ji_min_price = 50000;
        var ji_max_price = 199999;
    } else if (order_ji == 3) {
        var ji_min_price = 200000;
        var ji_max_price = 10000000;
    }
    if (order_price < ji_min_price) {
        if (reg2.test(langu)) {
            err_info('Minimum bet' + ji_min_price);
        } else {
            err_info('最小下单金额为' + ji_min_price);
        }

        return;
    }
    if (order_price > ji_max_price) {
        if (reg2.test(langu)) {
            err_info('Maximum bet' + ji_max_price);
        } else {
            err_info('最大下单金额为' + ji_max_price);
        }
    ...
    $.post(posturl, postdata, function (resdata) {
        ...
        if (resdata.type == 1) {
            ...
            if (resdata.data.ostyle == 0) {
                if (reg2.test(langu)) {
                    $('.pay_order_type').html('Rise');
                } else {
                    $('.pay_order_type').html('涨');
                }
                $('.pay_order_type').addClass('rise');
                $('.pay_order_type').removeClass('fall');
            } else {
                if (reg2.test(langu)) {
                    $('.pay_order_type').html('Fall');
                } else {
                    $('.pay_order_type').html('跌');
                }
