...
class User extends Base
{
	...
	public function userlist()
	{
		...
		if(isset($data['username']) && !empty($data['username'])){
			...
			$where['username|uid|utel|nickname'] = array('like',array('%'.$data['username'].'%','%'.base64_encode($data['username']).'%'),'OR'); //2018-11-13
	...
	public function useradd()
	{
		if(input('post.')){
			...
			$issetutl = db('userinfo')->where('utel',base64_encode($data['utel']))->find(); //2018-11-27
			if($issetutl){
	...
	public function userprice()
	{
		...
		if(isset($data['username']) && !empty($data['username'])){
			if($data['stype'] == 1){
				...
				$where['username|u.uid|utel|nickname'] = array('like',array('%'.$data['username'].'%','%'.base64_encode($data['username']).'%'),'OR'); //2018-11-27
			}
			if($data['stype'] == 2){
				//$puid = db('userinfo')->where(array('username'=>$data['username']))->whereOr('utel',$data['username'])->value('uid');
				$puid = db('userinfo')->where(array('username'=>$data['username']))->whereOr('utel',base64_encode($data['username']))->value('uid'); //2018-11-27
				if(!$puid) $puid = 0;
				$where['u.oid'] = $puid;
			}
	...
	public function vipuserlist()
	{
		...
		if(isset($data['username']) && !empty($data['username'])){
			//$where['username|uid|utel|nickname'] = array('like','%'.$data['username'].'%');
			$where['username|uid|utel|nickname'] = array('like',array('%'.$data['username'].'%','%'.base64_encode($data['username']).'%'),'OR'); //2018-11-27
			$getdata['username'] = $data['username'];
		}
	...
	public function vipuseradd()
	{
		...
		if(input('post.')){
			...
			$c_uid = Db::name('userinfo')->where('username',base64_encode($data['username']))->value('uid'); //2018-11-27
			if($c_uid){
	...
	*/
	...
	public function addprice()
	{
		...
		$post['utel'] = base64_encode(trim($post['utel'])); //2018-11-27
		...
		$user = db('userinfo')->where('utel',base64_encode($post['utel']))->find(); //2018-11-27

		if(!$user) return WPreturn('此用户不存在，请正确填写用户手机号',-1);

		//$ids = db('userinfo')->where('utel',$post['utel'])->setInc('usermoney',$post['bpprice']);
		$ids = db('userinfo')->where('utel',base64_encode($post['utel']))->setInc('usermoney',$post['bpprice']); //2018-11-27
	...
	//2018-11-27 数据库更新字段
	public function uputel00(){
		
		$userinfo=db('userinfo')->order('uid asc')->select();
		foreach ($userinfo as $key => $value) {
			dump($value['utel']);
			
			//$up['utel']=base64_encode($value['utel']);
			//Db::name('userinfo')->where("uid=".$value['uid'])->update($up);
		}
