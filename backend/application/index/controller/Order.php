...
class Order extends Base
{
	...
	public function addorder()
	{
		...
		*/
		if($data['order_price'] > $conf['order_max_price']){
			return WPreturn(lang('dbcczd').$conf['order_max_price'].'！',-1);
		}
		$allcount = Db::name('order')->where(array('ostaus'=>0,'uid'=>$data['uid']))->count();
		if($allcount >  $conf['max_order_count']){
			return WPreturn(lang('zdccsl').$conf['max_order_count'].'！',-1);
		}
		//验证是否开市
		...
		if($user['usermoney'] < $data['order_price'] + $web_poundage){
			return WPreturn(lang('yebz'),-1);
		}
		
		//验证金额 20 ~ 5000
		if($data['order_price'] < $conf['order_min_price']){
			return WPreturn(lang('bnxy').$conf['order_min_price'],-1);
		    
		}
		if($data['order_price'] > $conf['order_max_price']){
			return WPreturn(lang('bncg').$conf['order_max_price'],-1);
		...
		if($data['order_price'] < $conf['order_min_price'] || $data['order_price'] > $conf['order_max_price']){
			...
			return WPreturn('Sorry! Unknow Error!',-1); //英文作妖
		}
        ...
        if($ids){
        	...
        	if($editmoney){
        		...
        		return WPreturn(lang('xdcg'),-1);
        	}

        }else{
        	return WPreturn(lang('xdsb'),-1);
        }
		//dump($data);
	}
	...
	public function getchart()
    {
        ...
        $data['jiaoyilishi'] = lang('jyls');
        $data['chicangmingxi'] = lang('ccmx');
        $data['lishimingxi'] = lang('lsmx');
    ...
    public function lixibao()
    {
        ...
        $this->assign('shouyi',$shouyi?$shouyi:0);
        $this->assign('before_shouyi',$before_shouyi?$before_shouyi:0);
        $this->assign('shouyi_list',$shouyi_list);
        return $this->fetch();
    }

    public function lixibao_in()
    {
        $post = input('post.');
        $money = $post['money'];
        if(!$money){
            return WPreturn(lang('qsryxje'),-1);
        }
        $uid = $this->uid;;
        $user = Db::name('userinfo')->where('uid',$uid)->find();
        ...
        $newprice = $user['usermoney']-$money;
        $noemoney = $user['lixibao']+$money;
        Db::name('userinfo')->where('uid',$uid)->update(array('usermoney'=>$newprice));
        Db::name('userinfo')->where('uid',$uid)->update(array('lixibao'=>$noemoney));
        set_price_log($uid,2,$money,'利息宝','利息宝转入',0,$newprice);
        set_price_log($uid,1,$money,'利息宝','利息宝转入',0,$noemoney);
        return WPreturn('Succeful！',1);
    }

    public function lixibao_out()
    {
        $post = input('post.');
        $money = $post['money'];
        if(!$money){
        ...
        $uid = $this->uid;;
        $user = Db::name('userinfo')->where('uid',$uid)->find();
        if($user['lixibao'] < $money){
            return WPreturn(lang('jebz'),-1);
        }
        $newprice = $user['usermoney']+$money;
        $noemoney = $user['lixibao']-$money;
        Db::name('userinfo')->where('uid',$uid)->update(array('usermoney'=>$newprice));
        Db::name('userinfo')->where('uid',$uid)->update(array('lixibao'=>$noemoney));
        set_price_log($uid,1,$money,'利息宝','利息宝转出',0,$newprice);
        set_price_log($uid,2,$money,'利息宝','利息宝转出',0,$noemoney);
        return WPreturn('Succeful！',1);
    }



    public function mujuan()
    {
        $post = input('post.');
        $money = $post['money'];
        $zjpwd = $post['zjpwd'];
        if(!$money){
            return WPreturn('请输入有效金额!',-1);
        }
        if(!$zjpwd){
            return WPreturn('请输入资金密码!',-1);
        }
        $uid = $this->uid;;
        $user = Db::name('userinfo')->where('uid',$uid)->find();
        if($user['usermoney'] < $money){
            return WPreturn('金额不足!',-1);
        }
        if($user['paypwd'] != $zjpwd){
            return WPreturn('资金密码不正确!',-1);
        }
        $newprice = $user['usermoney']-$money;
        //$noemoney = $user['lixibao']+$money;
        Db::name('userinfo')->where('uid',$uid)->update(array('usermoney'=>$newprice));
        set_price_log($uid,2,$money,'募捐','募捐转出',0,$newprice);

        return WPreturn('感谢您的捐赠！',1);
    }
