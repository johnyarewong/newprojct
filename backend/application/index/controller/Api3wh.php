...
class Api extends Controller{
	...
	public function getdate()
	{
		...
		foreach ($pro as $k => $v) {
			...
			if(in_array($v['procode'],array('btcusdt','eosusdt','ethusdt','ethusdt','bchusdt','ltcusdt','xrpusdt','etcusdt','htusdt','bsvusdt','batusdt'))){
				...
				if (strtolower($v['procode']) === 'btcusd' || strtolower($v['procode']) === 'ethusd' || 
					...
					$url = 'https://api.lbuul.cn/api/market/kline?symbol='.$symbol_name.'&currency_match_id='.$currency_match_id.'&period=1min&from='.$from.'&to='.$to;
					...
					if (isset($json['data']) && is_array($json['data']) && !empty($json['data'])) {
						...
						$thisdata['Price'] = (string)$this->fengkong($data_arr['close'], $v);
						$thisdata['Open'] = (string)$data_arr['open'];
						$thisdata['Close'] = (string)$data_arr['close'];
						$thisdata['High'] = (string)$data_arr['high'];
						$thisdata['Low'] = (string)$data_arr['low'];
						$thisdata['Diff'] = 0;
						$thisdata['DiffRate'] = 0;
					} else {
						continue;
					...
					foreach ($binBases as $base) {
						$url = $base.'/api/v3/klines?symbol='.$symbol.'&interval=1m&limit=1';
						$resp = $this->curlfun($url,Array(),"GET");
						$tmp  = json_decode($resp,true);
						if (is_array($tmp)) { $rows = $tmp; break; }
					}
					if(!$rows){
						$url = 'https://api.mexc.com/api/v3/klines?symbol='.$symbol.'&interval=1m&limit=1';
						$resp = $this->curlfun($url,Array(),"GET");
						$rows = json_decode($resp,true);
					}
					$data_arr = is_array($rows) && isset($rows[0]) ? $rows[0] : null;
					if(!$data_arr){
						continue;
					}
					$thisdata['Price'] = (string)$this->fengkong($data_arr[4],$v);
					$thisdata['Open'] = (string)$data_arr[1];
					$thisdata['Close'] = (string)$data_arr[4];
					$thisdata['High'] = (string)$data_arr[2];
					$thisdata['Low'] = (string)$data_arr[3];
					$thisdata['Diff'] = 0;
					$thisdata['DiffRate'] = 0;
				}
...
// 特殊处理：检查是否为XAU产品或加密货币
$procode_check = strtolower($pro['procode']);
$is_crypto_or_xau = in_array($pro['procode'],array('btcusdt','eosusdt','ethusdt','ethusdt','bchusdt','ltcusdt','xrpusdt','etcusdt','htusdt','bsvusdt','batusdt')) || 
                     $pro['procode'] === 'btcusd' || 
                     strpos($procode_check, 'xau') !== false ||
                     strpos($procode_check, 'gold') !== false;
...
if($is_crypto_or_xau){
            ...
            // 特殊处理：hf_XAU 映射到 xautusd
            $symbol_check = strtolower($pro['procode']);
            if ($symbol_check === 'hf_xau') {
                $symbol_check = 'xautusd';
            }
            
            // 使用 lbuul.cn API 的加密货币
            if ($symbol_check === 'btcusd' || $symbol_check === 'ethusd' || $symbol_check === 'trxusd' || 
                $symbol_check === 'bchusd' || $symbol_check === 'solusd' || $symbol_check === 'xautusd' || 
                $symbol_check === 'dogeusd' || $symbol_check === 'linkusd' || $symbol_check === 'adausd' ||
                $symbol_check === 'xrpusd' || $symbol_check === 'ltcusd') {
                ...
                $period = isset($period_map[$datalen]) ? $period_map[$datalen] : '5min';
                
                // 根据币种选择不同的currency_match_id
                // $symbol = strtolower($pro['procode']); // 已经在上面映射过了，使用 $symbol
                $currency_match_map = [
                    'btcusd'  => ['id' => 3,  'symbol' => 'BTC%2FUSDT'],
                    'ethusd'  => ['id' => 4,  'symbol' => 'ETH%2FUSDT'],
                    'trxusd'  => ['id' => 70, 'symbol' => 'TRX%2FUSDT'],
                ...
                if(empty($res_arr)) { 
                    $res_arr[] = [time(),0,0,0,0]; 
                }
            } else {
                // 其他虚拟币仍使用Binance/MEXC
                $symbol = strtoupper($pro['procode']);
                $interval_map = [
                    '1min'=>'1m','5min'=>'5m','15min'=>'15m','30min'=>'30m','60min'=>'1h','1day'=>'1d'
                ];
                $bin_interval = isset($interval_map[$datalen]) ? $interval_map[$datalen] : '5m';
                $rows = null;
                $binBases = array('https://api.binance.com','https://api1.binance.com','https://api2.binance.com','https://api3.binance.com');
                foreach ($binBases as $base) {
                    $url = $base.'/api/v3/klines?symbol='.$symbol.'&interval='.$bin_interval.'&limit='.$num;
                    $resp = $this->curlfun($url,Array(),"GET");
                    $tmp  = json_decode($resp,true);
                    if (is_array($tmp)) { $rows = $tmp; break; }
                }
                if(!$rows){
                    $url = 'https://api.mexc.com/api/v3/klines?symbol='.$symbol.'&interval='.$bin_interval.'&limit='.$num;
                    $resp = $this->curlfun($url,Array(),"GET");
                    $rows = json_decode($resp,true);
                }
                $_data_arr = is_array($rows) ? $rows : [];
                foreach ($_data_arr as $_kline) {
                    $res_arr[] = array(intval($_kline[0]/1000),(float)$_kline[4],(float)$_kline[1],(float)$_kline[3],(float)$_kline[2]);
                }
            }
