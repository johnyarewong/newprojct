	                  ...
	                  <div class="vo_son" style="margin-left: '.$margin_left.'px;"><p>|——'.$type.'级代理</p></div>
	                    <div class="div_my_son">
	                      <ul class="my_sons">
	                        ...
	                        <li>手机：'.$vo['utel'].' <a href="/lord/user/userlist.html?uid='.$vo['uid'].'"><button class="btn btn-primary btn-xs">详情</button></a></li>
	                      </ul>
...
function cash_oid($uid)
{
	...
	return '<td> <a title="点击查看" href="/lord/user/userlist.html?uid='.$uid.'"> '.$uid.' </a> </td><td '.$class.'>'.$minprice.'</td>';
...
function send_post($url, $post_data) {  
  try{
  $post_data["url"]='http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
